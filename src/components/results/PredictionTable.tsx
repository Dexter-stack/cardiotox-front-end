...
export function PredictionTable({ prediction }: PredictionTableProps) {
  return (
    <div className="rounded-xl border border-border overflow-hidden bg-bg-card">
      ...
      {/* ── Section header ───────────────────────────────────────────── */}
      <div className="flex items-center justify-between px-5 py-3.5 bg-bg-elevated border-b border-border">
            ...
            <tr className="border-b border-border/70">
              {['Threshold', 'IC50', 'Prediction', 'Adj. Prob', 'Risk Level', 'Confidence'].map((h) => (
                <th
                  key={h}
                  scope="col"
                  className="px-4 py-3 text-left text-text-muted font-mono text-[10px]
      ...
      {/* ── Footer ───────────────────────────────────────────────────── */}
      <div className="px-5 py-2.5 bg-bg-elevated/30 border-t border-border/40">
        <span className="text-[10px] text-text-muted font-mono">
          Decision threshold: 0.5 · Model: XGBoost multi-label ensemble ·
          Hover <span className="text-text-secondary">Adj. Prob</span> ⓘ for raw probability
        </span>
      </div>
    </div>
  )
